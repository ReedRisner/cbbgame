...
export function simulatePossession(
  ...
  if (Math.random() < turnoverProb) {
    return { points: 0, turnover: true, foul: false, threeAttempt: false, made: false, reboundedOffense: false, assist: false, seconds: Math.max(10, Math.min(28, 24 - (scheme.possessionsPerGame / 12) + normalRandom(0, 2.3))) };
  }
  ...
  const foulProb = calculateFoulProbability(55 + defenseScheme.defAggRate * 45, 70, shotType === 'close' ? 0.95 : 0.65, { foulCallRate: gameState.refFoulRate ?? 1 });
  if (Math.random() < foulProb) {
    ...
    return { points, turnover: false, foul: true, threeAttempt: shotType === 'three', made: points > 0, reboundedOffense: false, assist: false, seconds: Math.max(10, Math.min(28, 24 - (scheme.possessionsPerGame / 12) + normalRandom(0, 2.3))) };
  }
  ...
  if (made) {
    return {
      ...
      seconds: Math.max(10, Math.min(28, 24 - (scheme.possessionsPerGame / 12) + normalRandom(0, 2.3))),
    };
  }

  const offRebProb = calculateOffensiveReboundProbability(45, 55, defenseScheme.zoneRate > 0.5 ? 0.92 : 1, 0.94 + normalRandom(0, 0.03));
